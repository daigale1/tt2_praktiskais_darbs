...
<div class="auth-card">
    ...
    @if($errors->any())
        <div style="background:var(--rose-lightest); border:1px solid var(--rose-deep); border-radius:8px; padding:10px 14px; margin-bottom:16px; font-size:13px; color:var(--rose-text);">
    ...
    <form action="{{ route('register') }}" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            <div class="form-group">
                <label for="location">Location <span style="font-size:11px; color:var(--tx3); font-weight:400;">(optional)</span></label>
                <input type="text" id="location" name="location"
                       value="{{ old('location') }}"
                       placeholder="e.g. Rīgas centrs">
            </div>
            <div class="form-group">
                <label for="age">Age <span style="font-size:11px; color:var(--tx3); font-weight:400;">(optional)</span></label>
                <input type="number" id="age" name="age"
                       value="{{ old('age') }}"
                       placeholder="e.g. 28" min="16" max="120">
            </div>
        </div>
        ...
        <p style="text-align:center; font-size:13px; color:var(--tx3); margin-top:16px;">
            ...
            <a href="{{ route('login') }}" style="color:var(--sage-mid); text-decoration:none; font-weight:500;">
                Sign in
            </a>
        </p>
    </form>
